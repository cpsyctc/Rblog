...
That made me realise that I was only "sort of" sure I understood that and reminded me that I have so far never used ellipses either as a way to describe 2D data or to the confidence intervals on two parameters from a model.  I decided to get to grips with this, starting by creating some correlated data.
...
A data ellipse creates an ellipse that is expected to contain the proportion of the joint population distributions of x and y derived from the observed data and the assumption of bivariation Gaussian distributions.  The area contained can be what you like really (within the logical restrictioins of it being a positive proportion/percentage and lower than 100%!)  Here are data ellipses for that dataset created with `car::dataEllipse()`.  I've used the default confidence intervals of 50% and 95%.
...
That has the 95% ellipse from the robust covariance matrix in green and the simple least squares ellipse in blue.  As you'd expect the difference is negligible as these are bivariate Gaussian data so there are few real outliers.
...
An ellipsoid hull is different from a data ellipse, it's an ellipse that contains all the observed points (with some on the boundary of the ellipse).  Here using `cluster::ellipsoidhull()` and base graphics.
...
That shows a joint distribution suggesting that the two estimated parameters are pretty much uncorrelated.  I think that doesn't have to be the case.  Let's try the very non-Gaussian joint distribution we get if we square both x and y.  Here's thge scattergram and 50% and 95% data ellipses for that.
