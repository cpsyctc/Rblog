...
## Bits for entry about transformations
...
## Plot for glossary entry about square root 
...
That clearly shows the bias: underestimating variance and that it decreases with sample size.
...
That shows that using the denominator of n - 1 corrects the bias and estimates the population variance (set at 5) without bias.
...
Add noise.  (I confess I tinkered with the seed to get what I wanted!)

```{r linearRegression4noisy}
set.seed(1)
nrow(tmpTib) -> n
...
### get the lm
lm(tmpTib2$y2 ~ tmpTib2$x) -> tmpLm
```

The intercept of that best fit linear regression is 
`r round(tmpLm$coefficients[1], 3)` and the slope is
`r round(tmpLm$coefficients[2], 3)`.

# Attenuation

This was just to create a plot and table for a glossary entry about attenuation by unreliability of measurement.

```{r attenuation1, layout="l-body-outset"}
set.seed(12345)
...
I think that shows clearly enough the impact of the added noise attenuating the correlation and reducing the slope
...
tmpTibSummary %>%
  ...
  write_csv(file = "attenuation.csv")
```

And that showed that the correction:
...
This was to illustrate the entry on Mokken scaling.

## Mokken double monotonicity model


```{r mokken1, layout="l-body-outset"}
set.seed(12345)
n <- 500000
...
tibble(theta = seq(-4, 4, .1)) %>%
  mutate(Item1 = irt_p(theta, a = 1, b = -1, c = 0),
         Item2 = irt_p(theta, a = 1, b = 1, c = 0)) %>%
  pivot_longer(cols = c(Item1, Item2),
               names_to = "Item",
               values_to = "p")-> tmpTib

ggplot(data = tmpTib,
       aes(x = theta, y = p, colour = Item)) +
  # geom_point() +
  geom_line(linewidth = 2) +
  ylab("Probability of answering 'yes' to item") +
  xlab("Person's true value on the latent variable") +
  ggtitle("Two item response curves, Mokken double monotonicity model",
          subtitle = "Reponse curves have same shape and don't cross") 
```

## Mokken monotone homogeneity model

```{r mokken2, layout="l-body-outset"}
tibble(theta = seq(-8, 8, .1)) %>%
  mutate(Item1 = irt_p(theta, a = 1, b = -1, c = 0),
         Item2 = irt_p(theta, a = .3, b = .1, c = 0)) %>%
  pivot_longer(cols = c(Item1, Item2),
               names_to = "Item",
               values_to = "p")-> tmpTib
...
ggplot(data = tmpTib,
  ...
  ylab("Probability of answering 'yes' to item") +
  xlab("Person's true value on the latent variable") +
  ggtitle("Two item response curves, Mokken monotone homogeneity model",
          subtitle = "Reponse curves still monotone but can differ in slope and can cross") 
```

# IRT (Item Response Theory)

This, leaning heavily on the above code for the Mokken scaling entry, was to give basics of IRT.

## One parameter model

```{r 1pIRT, layout="l-body-outset"}
tibble(theta = seq(-4, 4, .1)) %>%
  mutate(Item1 = irt_p(theta, a = 1, b = -1, c = 0),
         Item2 = irt_p(theta, a = 1, b = 1, c = 0)) %>%
  pivot_longer(cols = c(Item1, Item2),
...
tmpTib %>%
  mutate(delta = abs(p - .5)) %>%
  group_by(Item) %>%
  mutate(minDelta = abs(min(delta))) %>%
  filter(delta == minDelta) -> tibLocations
...
ggplot(data = tmpTib,
  ...
  geom_hline(yintercept = .5,
             linetype = 3) +
  geom_vline(data = tibLocations,
             aes(xintercept = theta,
                 colour = Item),
             linetype = 1) +
  ylab("Probability of answering 'yes' to item") +
  xlab("Person's true value on the latent variable") +
  ggtitle("Two item response curves, one parameter IRT model",
          subtitle = "Reponse curves have same shape but different locations shown by vertical reference lines") 

```

## Two parameter model

```{r 2pIRT, layout="l-body-outset"}
tibble(theta = seq(-8, 8, .1)) %>%
  mutate(Item1 = irt_p(theta, a = 1, b = -1, c = 0),
         Item2 = irt_p(theta, a = .3, b = .1, c = 0)) %>%
  pivot_longer(cols = c(Item1, Item2),
...
ggplot(data = tmpTib,
  ...
  ylab("Probability of answering 'yes' to item") +
  xlab("Person's true value on the latent variable") +
  ggtitle("Two item response curves, two parameter IRT model",
          subtitle = "Reponse curves still monotone but different locations shown by vertical reference lines\nand different slopes") 
```

## Three parameter model

```{r 3pIRT, layout="l-body-outset"}
tibble(theta = seq(-8, 8, .1)) %>%
  mutate(Item1 = irt_p(theta, a = 1, b = -1, c = 0),
         Item2 = irt_p(theta, a = .3, b = .1, c = 0.25)) %>%
  pivot_longer(cols = c(Item1, Item2),
               names_to = "Item",
               values_to = "p")-> tmpTib
...
ggplot(data = tmpTib,
  ...
  ylab("Probability of answering 'yes' to item") +
  xlab("Person's true value on the latent variable") +
  ggtitle("Two item response curves, three parameter IRT model",
          subtitle = "Reponse curves differ in location, slope and 'guessing' rate") +
  labs(caption = "Item2 has .25 guessing rate, Item1 has zero guessing rate ") +
...
funRasch <- function(p, i){
  ### this is basically the logistic function
  diff <- p - i
  pr <- exp(diff) / (1 + exp(diff))
}

n <- 5000 # to get nice smooth lines

tibble(person = seq(-5, 8, length.out = n)) %>% # sets range of person locations
  ### and now create probabilities per person for two items
  mutate(Item_1 = funRasch(person, 1),
         Item_2 = funRasch(person, 2)) -> tmpTib

### pivot longer to make nice plot easier
tmpTib %>%
  pivot_longer(cols = -person,
               names_to = "Item",
               values_to = "prob") -> tmpTibLong

### find the locations for each item at which probability
### is nearest to .5
tmpTibLong %>%
  mutate(delta = abs(prob - .5)) %>%
  group_by(Item) %>%
  mutate(minDelta = abs(min(delta))) %>%
  filter(delta == minDelta) -> tibLocations
...
ggplot(data = tmpTibLong,
  ...
  geom_line(linewidth = 2) +
  ### show the item locations, first the .5 reference line
  geom_hline(yintercept = .5,
             linetype = 3,
             linewidth = 1.5) +
  ### and now the locations
  geom_vline(data = tibLocations,
             aes(xintercept = person,
                 colour = Item),
             linetype = 2,
             linewidth = 1.5) +
  ylab("Probability of person answering 'Y' to Item") +
  scale_x_continuous("Person's location on scale of variable to be measured",
                     breaks = -5:8) +
  ggtitle("Probability of answering positively to two perfect Rasch model items",
       subtitle = paste0("Horizontal reference line p = .5",
                         "\nVertical reference lines give locations of the items")) +
  ### increase size of text for the web
...
```{r oddsratios1}
getOdds <- function(prob) {
  prob / (1 - prob)
  }
getOdds <- Vectorize(getOdds)
tibble(prob = seq(0.001, .999, .005)) %>%
  mutate(odds = getOdds(prob),
         logOdds = log(odds),
         diff = odds - prob) -> tmpTib

tmpTib %>%
  pivot_longer(cols = c(odds, logOdds)) %>%
  mutate(transform = if_else(name == "odds",
                             "raw",
                             "log"),
         refVal = if_else(transform == "raw",
                          1,
                          0)) -> tmpTibLong
...
ggplot(data = tmpTib,
  ...
  xlab("Probability") +
  ylab("Odds ratio") +
  ggtitle("Plot of odds ratio against probability")

ggsave(filename = "ods_vs_prob.png",
       width = 1700,
       height = 1470,
       units = "px")
```
...
ggplot(data = tmpTib,
  ...
  xlab("Odds ratio") +
  ylab("Probability") +
  ggtitle("Plot of probability against odds ratio")

ggsave(filename = "prob_vs_odds.png",
       width = 1700,
       height = 1470,
       units = "px")
```
...
ggplot(data = tmpTibLong,
  ...
  xlab("Probability") +
  ylab("Value (odds ratio or log odds)") +
  ggtitle("Plot of odds ratio and log odds ratio against probability")

ggsave(filename = "ods_and_logOdds_vs_prob.png",
       width = 1700,
       height = 1470,
       units = "px")
```
