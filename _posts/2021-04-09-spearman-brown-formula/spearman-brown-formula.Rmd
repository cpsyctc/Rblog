...
As announced [here](https://www.psyctc.org/Rblog/posts/2021-02-10-making-my-first-usable-package/), there is a developing CECPfuns package (https://github.com/cpsyctc/CECPfuns) which contains the function `getRelBySpearmanBrown` and which was used in the code above.  There is also the function `SpearmanBrown` in the [psychometric package](https://cran.r-project.org/web//packages/psychometric/) by Thomas D. Fletcher which does the same thing (and gives the same results!)
