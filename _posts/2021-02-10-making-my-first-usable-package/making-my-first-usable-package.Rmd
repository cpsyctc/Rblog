...
# Using github pages and pkgdown to create a page about the package

*added 6.viii.25 after realising that I'd forgotten most of how this works*
Back in 2021 (it seems, amazing what I can forget in four years) I seem to have tweaked my github set up for CECPfuns to add a gh-pages branch. That is created using the [pkgdown package](https://pkgdown.r-lib.org/) and it uses things in the package itself to generate the static site about the package that is visible at https://cecpfuns.psyctc.org/index.html.  The crucial thing is a _pkgdown.yml file created in the package root which contains:

```
url: https://cecpfuns.psyctc.org/
template:
  bootstrap: 5
```

I think github looks after using that url and I can also see that I had to create a CNAME entry in the DNS for psyctc.org that maps to cpsyctc.github.io so that when we go to https://cecpfuns.psyctc.org/ the DNS looks after actually taking content from cpsyctc.github.io.  The whole process seems to happen in seconds to minutes after I push the latest changes to the package to github.  I need to do a bit more sleuthing to make a better set of pages at https://cecpfuns.psyctc.org/ but they're not bad.  As with the package generally, you do have to ...

# ... remember to update NEWS.md and DESCRIPTION *every* time you update the package!

*added 6.viii.25*
Put a new entry and updated version number in NEWS.md and make sure that the version number is also updated in DESCRIPTION Christopher! Git will look after pretty much all the other bits of curating I think.
...
* *6.viii.25 Added sectiona about github pages and pkgdown and a trailing section reminding me to update NEWS.rd and DESCRIPTION every time I change anything!*
