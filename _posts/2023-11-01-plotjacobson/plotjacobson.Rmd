...
### can now build the default subtitle
  ...
  if(nameRCIDetOnly){
    tmpTib %>%
      ...
      mutate(pointNames = if_else(RCIchangeNum != -1, "", pointNames)) -> tmpTib
  }
...
Here I'm showing the call to the function to make some improvements to that plot.

```{r defaultPlotImproved, echo = TRUE, layout="l-page"}
getJacobsonPlot(baselineScores = tibDat$firstScore, 
                finalScores = tibDat$lastScore, 
                CSC = 1.26, 
                RCI = 0.3983001,
                minPossScore = 0, 
                maxPossScore = 4,
                title = "Jacobson plot of mixed data, real and artificial", # changed the title
                xLab = "Baseline score, full CORE-OM, total score", # changed the x axis label
                pointColourScheme = "RCI3", # map RCI to point colours
                pointShapeScheme = "RCI3", # adds mapping of RCI to point shapes
                legendNamePoints = "Point mapping: RCI",  # adds sensible title to the legend
                noLegend = FALSE, # switch legends on
                pointSize = 1.8, # crucially, push the point size up to make them more visible
                lineTypeRCI = 3, # use dotted lines for the RCI tramlines
                labelCSClines = TRUE) # label the CSC values onto the CSC lines 
...
4. The RCSC categories that use both RCI and CSC:  
   1. RCSC5: NoRelChange, RelDet, RelImpHighToLow ("Recovered"), RelImpHighToHigh, RelImpLowToLow (using the obvious abbreviations and assuming negatively cued measure) 
   2. RCSC7: RelDetHighToHigh, RelDetLowToHigh, RelDetLowToLow, NoRelChange, RelImpHighToHigh, RelImpHighToLow ("Recovered"), RelImpLowToLow  
   3. RCSC10: RelDetHighToHigh, RelDetLowToHigh, RelDetLowToLow, NoRelChangeHighToHigh, NoRelChangeLowToHigh, NoRelChangeHighToLow, NoRelChangeLowToLow, RelImpHighToHigh, RelImpHighToLow ("recovered"), RelImpLowToLow.
...
## The arguments for `getJacobsonPlot()`

Here is the full set of arguments.  You'll see that most have default values.  I'll go through all the arguments below, by their groups, after demonstrating some of the category mappings.
...
That coincidentally introduces another argument: areaAlpha. To me, more so on some screens than others, the red points are visible on top of the red area but the green points are not very visible.  There are two arguments that control the alpha of the points and areas: pointAlpha and areaAlpha. The default value of pointAlpha is 1, i.e. no transparency, the default value of areaAlpha is .3, i.e. 30% transparency which for my vision system at least is working for the red "deterioration" but not the green "improvement".  Let's trying pushing the area transparency down to .1.
...
## Going through the category systems that can be used
...
As we just saw the subtitle argument allows you to add a subtitle using the default formatting of subtitles for the theme.  This next shows the impacts of the title, titleJustificn, xLab and yLab arguments.  Why anyone might want to use right justified title (and therefore subtitle as I've bundled them together for justification) beats me but I think some journals do require left justifcation so titleJustificn = 0 would get that.
...
```{r textArguments, eval=FALSE}
getJacobsonPlot(baselineScores = tmpTib$firstScore, 
             finalScores = tmpTib$lastScore, 
             CSC = 1.26, 
             RCI = 0.3983001,
             minPossScore = 0, 
             maxPossScore = 4,
             title = "Jacobson (RCSC) plot of data from Quito clinic",
             subtitle = paste0("n = ", row(tmpTib), " clients"),
             titleJustificn = 1,
...
The arguments are lineTypeCSC, lineColourCSC, lineWidthCSC for the horizontal and vertical CSC references lines; lineTypeNoChange, lineColourNoChange, lineWidthNoChange for the diagonal no change line and lineTypeRCI, lineColourRCI, lineWidthRCI for the RCI tramlines.

This truly aesthetically horrible plot has removed the RCI tramlines by using lineTypeRCI = 0, turned the CSC lines red with lineColourCSC = "red" and dotted with lineTypeCSC = 3.  I suspect that these options are only really useful when making plots to use for teaching.

```{r lineTypes, eval=FALSE}
getJacobsonPlot(baselineScores = tmpTib$firstScore, 
             finalScores = tmpTib$lastScore, 
             CSC = 1.26, 
             RCI = 0.3983001,
...
The pertinent arguments are pointNames = NA, 
             namesNotPoints = FALSE, 
             nameRCIDetOnly = FALSE, 
             namesSize = 2, 

```{r pointIDs1, eval=FALSE}
getJacobsonPlot(baselineScores = tmpTib$firstScore, 
             finalScores = tmpTib$lastScore, 
             pointNames = NA, 
             namesNotPoints = FALSE, 
...
## ls()
```
 [1] "areaAlpha"                         "areaColourScheme"                 
 [3] "baselineScores"                    "coloursCSC4"                      
 [5] "coloursDirection"                  "coloursRCI3"                      
 [7] "coloursRCSC10"                     "coloursRCSC5"                     
 [9] "coloursRCSC7"                      "Complete"                         
[11] "CSC"                               "dfPolygonNoRelChangeDetHighToHigh"
[13] "dfPolygonNoRelChangeDetLowToHigh"  "dfPolygonNoRelChangeDetLowToLow"  
[15] "dfPolygonNoRelChangeImpHighToHigh" "dfPolygonNoRelChangeImpHighToLow" 
[17] "dfPolygonNoRelChangeImpLowToLow"   "dfPolygonRelDetHighToHigh"        
[19] "dfPolygonRelDetLowToHigh"          "dfPolygonRelDetLowToLow"          
[21] "dfPolygonRelImpHighToHigh"         "dfPolygonRelImpHighToLow"         
[23] "dfPolygonRelImpLowToLow"           "finalScores"                      
[25] "goodScorePos"                      "greyScale"                        
[27] "Incomplete"                        "labelAllPolygons"                 
[29] "labelCSClines"                     "labelsCSC4"                       
[31] "labelsDirection"                   "labelsRCI3"                       
[33] "labelsRCSC10"                      "labelsRCSC10pos"                  
[35] "labelsRCSC5"                       "labelsRCSC5pos"                   
[37] "labelsRCSC7"                       "labelsRCSC7pos"                   
[39] "legendNameAreas"                   "legendNamePoints"                 
[41] "lineColourCSC"                     "lineColourNoChange"               
[43] "lineColourRCI"                     "lineTypeCSC"                      
[45] "lineTypeNoChange"                  "lineTypeRCI"                      
[47] "lineWidthCSC"                      "lineWidthNoChange"                
[49] "lineWidthRCI"                      "listArgs"                         
[51] "maxPossScore"                      "minPossScore"                     
[53] "nameRCIDetOnly"                    "namesNotPoints"                   
[55] "namesSize"                         "nHelpSeeking"                     
[57] "noLegend"                          "noMessages"                       
[59] "noWarnings"                        "oldWarn"                          
[61] "omitNHS"                           "Original_n"                       
[63] "pointAlpha"                        "pointColourScheme"                
[65] "pointNames"                        "pointShape"                       
[67] "pointSize"                         "RCI"                              
[69] "scoreRange"                        "pointShapeNoRelChange"            
[71] "pointShapeRelDet"                  "pointShapeRelImp"                 
[73] "pointShapeScheme"                  "subtitle"                         
[75] "tibAreas"                          "tibAreas1"                        
[77] "title"                             "titleJustificn"                   
[79] "tmpPlot"                           "tmpTib"                           
[81] "tmpTibMissing"                     "tmpVecRCSC10"                     
[83] "tmpVecRCSC5"                       "tmpVecRCSC7"                      
[85] "vec12alphas"                       "vec12colours"                     
[87] "vecAreaColours"                    "vecAreaLabels"                    
[89] "vecAreas"                          "vecPointColours"                  
[91] "vecPointLabels"                    "vecPointShapes"                   
[93] "vecPointsRCSC5"                    "warningsToErrors"                 
[95] "xLab"                              "yLab"  
```

## tibAreas
```
 [1] "iPolygon"    "namePolygon" "RCI"         "direction"   "start"       "end"         "CSC4"       
 [8] "areaAlpha"   "RCSC5"       "RCSC7"       "RCSC10"      "x"           "y"           "xCentre"    
[15] "yCentre"     "areas"      
```
## tmpTib
```
 [1] "pointNames"      "firstScore"      "lastScore"       "goodScorePos"    "RCIcrit"        
 [6] "CSCcrit"         "firstLastChange" "direction3"      "RCIchange"       "firstCSCstatus" 
[11] "lastCSCstatus"   "CSC4"            "RCSC5"           "RCSC7"           "RCSC10"         
[16] "Shapes"          "Points" 
```
The plotting by actually uses all 12 areas/polygons that cover the entire RCSC plot.  These are as follows, spiralling in clockwise from top to the right of the CSC (tdc: to dead centre?!)
...
You can see these using the `labelAllPolygons = FALSE` argument to `getJacobsonPlot()`
...
getJacobsonPlot(baselineScores = tibDat$firstScore, finalScores = tibDat$lastScore, 
             ...
             greyScale = FALSE)
```
```{r rubbishPlot, eval=FALSE}
### just to restart the plot area
# ggplot(data = tibble(x = letters[15:20], y = letters[15:20]), aes(x = x, y = y)) + geom_point(aes(shape = x), colour = "blue", size = 7)
# 
# ggplot(data = tibble(x = letters[23:25], y = letters[23:25], shape = c(23L, 24L, 25L)), aes(x = x, y = y)) + geom_point(aes(shape = shape), colour = "blue", size = 9)
# 
ggplot(data = tibble(x = 23:25, y = 23:25, z = letters[1:3]), aes(x = x, y = y, shape = z)) + geom_point(fill = "blue", size = 9) + scale_shape_manual(values = 23:25)
# ggplot(data = tibble(x = as.integer(1:10), y = as.integer(1:10)), aes(x = x, y = y)) + geom_point() + geom_line()

### reprex showing the issue
# library(tidyverse)
...
```
```{r ggnewscale_option, eval=FALSE}
### sorting it (longwindedly) with ggnewscale
# library(tidyverse)
# library(ggnewscale) # allows me to change the scales used
# tibble(x = 2:9, y = 2:9, 
#        ### I have used A:C to ensure the changes sort in the correct order to avoid the messes of using shape to scale an ordinal variable
#        ### have to say that seems a case where it is perfectly sensible to map shapes to an ordinal variable, scale_shape_manual() makes
#        ### this difficult hence this bodge
#        c = c(rep("A", 5), "B", rep("C", 2)),
#        change = c(rep("Deteriorated", 5), "No change", rep("Improved", 2))) %>%
#   ### this is just keeping the original coding but not used below
#   mutate(change = ordered(change,
#                           levels = c("Deteriorated", "No change", "Improved"))) -> tmpTibPoints
...
#   scale_color_manual(values = pts_colors) -> tmpPlot
# 
# ### this confirms that the option won't work for PDF output from pdf()
# pdf(file = "utf_option.pdf")
# tmpPlot
# dev.off()
# 
# ### does work for jpeg()
# jpeg(file = "utf_option.jpg")
# tmpPlot
# dev.off()
# 
# ### does work for png()
# png(file = "utf_option.png")
# tmpPlot
# dev.off()
```
```{r gggrid_option, eval=FALSE}
### this was just be exploring the gggrid option from https://coolbutuseless.github.io/2021/11/04/custom-ggplot2-point-shapes-with-gggrid/
# library(grid)
# library(gggrid)
# 
# hex <- FALSE
...
#     gnew$vp$width  <- unit(point[['size']]/4, 'cm')
#     gnew$vp$height <- unit(point[['size']]/4, 'cm')
...
# 
# ### for comparison here is the simple geom_point() plot
# ggplot(
#   data = mtcars, 
#   mapping = aes(
#     x      = mpg, 
...
* check that all those mappings are correct, then recheck with goodScorePos TRUE
