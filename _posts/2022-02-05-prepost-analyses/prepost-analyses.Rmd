...
### found the answer at https://stackoverflow.com/questions/27611438/density-curve-overlay-on-histogram-where-vertical-axis-is-frequency-aka-count
### it involves using a multiplier which is n * binwidth
tmpBinWidth = .5

getGaussianCounts <- function(vals, binCount){
  ### function that finds the best fitting Gaussian for an empirical vector of values
  ### and converts the density values from dnorm() to the appropriate counts given the
  ### binwidth being used to plot the ggplot::geom_histogram()
  vals <- na.omit(vals)
  n <- length(vals)
  meanvals <- mean(vals)
  sdvals <- sd(vals)
  ### now work out the points on the x axis for the histogram with that binwidth
  x <- seq(min(vals), max(vals), length = binCount)
  binWidth <- x[2] - x[1]
  mult <- n * binWidth
  print(x)
  dnorm(x, meanvals, sdvals) * mult
}
...
ggplot(data = tibDat,
  ...
  geom_function(fun = getGaussianCounts,
                args = list(binCount = tmpBins),
                colour = "green",
                size = 1) #+
